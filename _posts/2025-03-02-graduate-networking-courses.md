...
A few months ago I was debugging an issue where pods in one AKS node pool couldn't reach pods in another. The network policies looked correct. The NSG rules were fine. The issue turned out to be a conflict between Azure CNI's IP allocation and a subnet mask that was too small for the number of pods we were scheduling.
